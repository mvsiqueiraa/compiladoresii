...
public class Parser {
	...
	// 	return e;
	// }
	// expr agora lida com o OR, chamando andExpr para maior precedência
	private Expr expr() {
		Expr e = andExpr(); // Chamada para a próxima precedência (AND)
		while( look.tag() == Tag.OR ) {
			move();  
			e = new Or(e, andExpr()); // Usa andExpr
		}
		return e;
	}

	// Novo método para o operador lógico AND
	// andExpr lida com o AND, chamando rel para maior precedência
	private Expr andExpr() {
		Expr e = rel(); // Chamada para a próxima precedência (Relacionais)
		while ( look.tag() == Tag.AND ) { // Operador AND
			move();
			e = new And(e, rel()); // Cria um nó And
		}
		return e;
	}
	...
	private Expr rel() {
		...
		while ( look.tag() == Tag.LT || 
				look.tag() == Tag.LE ||
				look.tag() == Tag.GT ||
				look.tag() == Tag.EQ || // Adicionado ==
				look.tag() == Tag.NE || // Adicionado !=
				look.tag() == Tag.GE ) { // Adicionado >=
			Token op = move();
